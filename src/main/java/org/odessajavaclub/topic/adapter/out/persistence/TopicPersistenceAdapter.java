...
public class TopicPersistenceAdapter
  ...
  @Override
  public List<Topic> listByStatus(
  ...
  @Override
  public List<Topic> listByDateRange(
  ...
  public List<Topic> listByDate(Date start, String sortBy, String order, int page, int size) {
    ...
    PageRequest pageRequest =
        PageRequest.of(page, size, Sort.by(Sort.Direction.fromString(order), sortBy));
    return topicJpaRepository.findByEventBetween(start, java.sql.Date.valueOf(endDate), pageRequest)
        .stream()
        .map(topicEntityMapper::toTopic)
        .collect(Collectors.toList());
  }
  ...
  @Override
  public List<Topic> listByTitleLike(
      String title, String sortBy, String order, int page, int size) {
    PageRequest pageRequest =
        PageRequest.of(page, size, Sort.by(Sort.Direction.fromString(order), sortBy));
  ...
  public boolean deleteTopic(Topic.TopicId topicId) {
    return topicJpaRepository
        .findById(topicId.getValue())
        .map(
            topic -> {
